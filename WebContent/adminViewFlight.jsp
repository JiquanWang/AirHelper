...
<header>
    ...
    <div>
    <form action="/AirHelper/adminViewFlight.action" method="post" class="large-4 large-offset-4">
    	<label>航班号</label>
        <input type="text" name="flightNo" value="${f.flightNo }">               
        <label>起飞时间区间</label>
        <input type="text" id="temp" name="startTime1" value="${f.startTime1 }" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
        <input type="text" id="temp" name="endTime1" value="${f.endTime1 }" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
        <label>起点</label>
    ...
    <c:if test="${flights != null }">
    	...
    	<c:forEach items="${flights }" var="flight">
    		<tr>    		
				...
				<td><form action="/AirHelper/adminDeleteFlight.action" method="post">
				...
				<input class="small radius button left"type="submit" value="删除航班"></form></td>				
			</tr>	
    	</c:forEach>
    </table>
    </c:if>
